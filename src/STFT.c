...
/*--------------------------------------------------------------------*/
/*--------------------------------------------------------------------*/
struct Opaque_STFT_Autocorr_Struct
{
  int           should_resynthesize;
  int           window_size;
  int           fft_N;
...
/*--------------------------------------------------------------------*/
STFT_Autocorr* stft_autocorr_new(int window_size /*power of 2 please*/, int overlap)
{
  STFT_Autocorr* self = calloc(1, sizeof(*self));
  if(self != NULL)
    {
      self->window_size          = window_size;
      ...
      if(self->window           == NULL) return stft_autocorr_destroy(self);
      if(self->running_output   == NULL) return stft_autocorr_destroy(self);
      if(self->real             == NULL) return stft_autocorr_destroy(self);
      if(self->imag             == NULL) return stft_autocorr_destroy(self);
...
/*--------------------------------------------------------------------*/
STFT_Autocorr* stft_autocorr_destroy(STFT_Autocorr* self)
{
  if(self != NULL)
    {
      if(self->window != NULL)
  ...
  return (STFT_Autocorr*) NULL;
}

/*--------------------------------------------------------------------*/
void stft_autocorr_process(STFT_Autocorr* self, dft_sample_t* real_input, int len, stft_autocorr_onprocess_t onprocess, void* onprocess_self)
{
  int i, j;
